...
清洗内容:
  ...
  2. 去重（文件内 + 跨文件，按 input+output 去重）
  3. 科室过滤（移除非医学科室数据）
...
def write_by_department(records: list[dict], output_dir: Path, fmt: str, keep_dept: bool):
...
def main():
    parser = argparse.ArgumentParser(description="医疗数据清洗：GBK CSV → JSON 指令格式")
    ...
    parser.add_argument("--limit", type=int, default=None,
                        help="每个文件最多处理多少条（测试用）")
    parser.add_argument("--keep-dept", action="store_true",
    ...
    for csv_path in csv_files:
        ...
        print(f"  过短过滤: {stats['filtered_short']}")
        print(f"  有效保留: {len(records)}")

    # 3. 跨文件去重
    print(f"\n===== 去重 =====")
    print(f"  去重前: {len(all_records)}")
    all_records, dup_count = deduplicate(all_records)
    ...
    if args.split_by_dept:
        ...
        file_list = write_by_department(all_records, split_dir, args.format, args.keep_dept)
    ...
    print(f"  过短过滤:     {total_stats['filtered_short']}")
    print(f"  重复删除:     {dup_count}")
